...
/*
  ...
  Author URI: http://www.niftycart.com
  Text Domain: nifty-buy-button
  License: GPLv2 or later
  ----------------------------------------------------------------------
  Copyright 2016 Listening Software Inc.
 */
...
define( 'NIFTY_BUY_BUTTON_VERSION', '3.4' );
...
define( 'NBB_SECURE_CART_API_URI', 'https://1securecart.net/GetCatalogItems.asp?APIKey=' );
define( 'NBB_ORDER_NOW_BUTTON_API', 'https://1securecart.net/orderbutton.asp' );
...
if ( !class_exists( 'Nifty_Buy_Button' ) ) {
	...
	class Nifty_Buy_Button {
		...
		public function include_classes() {
			require_once 'includes/nbb-settings.php';
		}
		...
		public function register_nbb_admin_menu() {
			add_menu_page(
					  __( 'Nifty Buy Button', 'nifty-buy-button' ), __( 'Nifty Buy Button', 'nifty-buy-button' ), 'manage_options', 'nifty-buy-button', array( 'NBB_Settings', 'nbb_nifty_buy_button_menu_page' ), 'dashicons-editor-kitchensink', 26
			);
		}
		...
		public function do_nbb_nifty_buy_button_shortcode( $atts ) {
			$atts = shortcode_atts( array(
				 'url' => NBB_ORDER_NOW_BUTTON_API . '?ItemId=10',
				 'align' => 'center',
				 'wrap' => 'no'
					  ), $atts, 'bartag' );

			if ( empty( $atts['url'] ) ) {
				$atts['url'] = NBB_ORDER_NOW_BUTTON_API . '?ItemId=10';
			}
		...
		public function do_niftycart_shortcode( $atts ) {
			...
			foreach ( $product_ids as $key => $product_id ) {
				...
				$product_api = NBB_ORDER_NOW_BUTTON_API . '?ItemId=' . $product_id;
